...
def get_corner_points(data_path, timestamp, confidence=0.2):
    ...
    for id, box in enumerate(boxes):
        ...
        cv2.imwrite(os.path.join(data_path, 'output', timestamp,
                    f'AfterThreshold_{id}_improved.jpg'), crop_gray)
        # h,w = crop_gray.shape
        ...
        # save contours
        black_img = np.zeros(
            (crop_img.shape[0], crop_img.shape[1], 3), np.uint8)
        cv2.drawContours(black_img, contours, -1, (255, 255, 255), 1)
        cv2.imwrite(os.path.join(data_path, 'output', timestamp,
                    f'contours_{id}_improved.jpg'), black_img)
    ...
    cv2.imwrite(os.path.join(data_path, 'output',
                timestamp, 'points_improved.jpg'), img)
    cv2.imwrite(os.path.join(data_path, 'output',
                timestamp + '_points_improved.jpg'), img)
